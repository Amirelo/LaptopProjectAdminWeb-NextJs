...
export default function ProcessorPage() {
  ...
  const onRevertPressed = async (item) => {
    const deletePress = await updateProcessor(
      item.processorID,
      item.name,
      item.CPU_Speed,
      item.cores,
      item.logicalProcessor,
      item.cacheMemory,
      1
    );
    setCurrentItem({});
  ...
  const onConfirmDeletePressed = async () => {
    const deletePress = await updateProcessor(
      currentItem.processorID,
      currentItem.name,
      currentItem.CPU_Speed,
      currentItem.cores,
      currentItem.logicalProcessor,
      currentItem.cacheMemory,
      0
    );
    setCurrentItem({});
    setShowDeleteTab(false);
            ...
            .map((processor) => {
              return (
                ...
                >
                  ...
                  <td className="text-center border">
                    {checkStatus(processor.logicalProcessor)}
                  </td>
                  <td className="text-center border">
                    {checkStatus(processor.cacheMemory)}
                  </td>

                  <td className="text-center border">
                    <button
                      onClick={() => onEditIconPressed(processor)}
                  ...
                  {processor.status != 0 ? (
                    <td className="text-center border">
                      <button
                        onClick={() => onDeleteIconPressed(processor)}
                    ...
                    <td className="text-center border">
                      <button
                        onClick={() => onRevertPressed(processor)}
