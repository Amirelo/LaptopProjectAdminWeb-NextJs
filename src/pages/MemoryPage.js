...
export default function MemoryPage() {
  ...
  const onRevertPressed = async (item) => {
    const deletePress = await updateMemory(
      item.memoryID,
      item.currentRAM,
      item.type,
      item.speed,
  ...
  const onConfirmDeletePressed = async () => {
    const deletePress = await updateMemory(
      currentItem.memoryID,
      currentItem.currentRAM,
      currentItem.type,
      currentItem.speed,
  ...
  const dataSort = (list) => {
    ...
    if (searchText != null) {
      myList = myList.filter((item) =>
        (item.currentRAM + " " + item.type + " " + item.speed)
          .toLowerCase()
              ...
              return (
                ...
                >
                  ...
                  {memory.status != 0 ? (
                    <td className="text-center border">
                      <button
                        onClick={() => onDeleteIconPressed(memory)}
                    ...
                    <td className="text-center border">
                      <button
                        onClick={() => onRevertPressed(memory)}
