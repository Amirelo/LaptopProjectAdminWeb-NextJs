...
export default function StoragePage() {
    ...
    const onRevertPressed = async (item) => {
        const deletePress = await updateStorage(item.storageID,item.type, item.maxSlots, item.availableSlots,item.currentStorage, 1);
        setCurrentItem({})
    ...
    const onConfirmDeletePressed = async () => {
        const deletePress = await updateStorage(currentItem.storageID, currentItem.type,currentItem.maxSlots, currentItem.availableSlots, currentItem.currentStorage, 0);
        setCurrentItem({})
        setShowDeleteTab(false)
        console.log(deletePress)
        setDataChange(!dataChange);
        ...
        isLoading == false ?
            <>
                ...
                {showDeleteTab ?
                    ...
                    {showEditTab ?
                        <EditStorageTab item={currentItem} onBackgroundPressed={() => setShowEditTab(false)} onDeletePress={onFinishedHandlingItem} />
                        : <></>}
                            ...
                            return (
                                <tr className={storage.status == 0 ? "bg-slate-500/25" : "even:bg-sky-50"}  key={storage.storageID}>
                                    ...
                                    <td className="text-center border">
                                        <button onClick={() => onEditIconPressed(storage)} className="hover:text-reviewColor transition-colors">
                                    ...
                                    {storage.status != 0 ?
                                        <td className="text-center border">
                                            <button onClick={() => onDeleteIconPressed(storage)} className="hover:text-cancelColor transition-colors">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        ...
                                        :
                                        <td className="text-center border">
                                            <button onClick={() => onRevertPressed(storage)} className="hover:text-acceptColor transition-colors">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
