...
export default function OrderPage() {
  ...
  const dataSort = (list) => {
    ...
    switch (sortType) {
      default:
        myList.sort((a, b) => b.userOrderID - a.userOrderID);
    }
    console.log(myList);
        ...
        <table className="w-full mt-10">
          ...
          {listSort
            ...
            .map((userOrder) => {
              return (
                <tr
                  className="even:bg-backgroundInputColor/25"
                  key={userOrder.userOrderID}
                >
                  ...
                  <td className="text-center border">
                    {priceFormat(userOrder.shippingFee)}
                  </td>
                  <td className="text-center border">
                    {priceFormat(userOrder.totalPrice)}
                  </td>
                  <td
                    className={`border text-center font-medium ${
                      userOrder.status == 1
                        ? "animate-bounce text-processColor"
                        : userOrder.status == 0
                        ? "text-errColor"
                        : userOrder.status == 3
                        ? "text-warnColor"
                  ...
                  >
                    {userOrder.status < 4 ? (
                      <button
                        onClick={() => onEditIconPressed(userOrder)}
                        className="hover:font-bold"
                      >
                        {statusArr[userOrder.status]}
                      </button>
                    ) : (
