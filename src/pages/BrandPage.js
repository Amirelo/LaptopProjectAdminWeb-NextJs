...
export default function BrandPage() {
  ...
  const onConfirmDeletePressed = async () => {
    const deletePress = await updateBrandByID(
      currentItem.brandID,
      currentItem.name,
      0
    );
          ...
          {listSort
            ...
            .map((brand) => {
              return (
                <tr
                  className={
                    brand.status == 0 ? "bg-slate-500/25" : "even:bg-backgroundInputColor/25"
                  }
                  key={brand.brandID}
                >
                  ...
                  <td className="text-center border">
                    <button
                      onClick={() => onEditIconPressed(brand)}
                      className="hover:text-warnColor transition-colors"
                    >
                      <svg
                  ...
                  {brand.status != 0 ? (
                    <td className="text-center border">
                      <button
                        onClick={() => onDeleteIconPressed(brand)}
                        className="hover:text-errColor transition-colors"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                    ...
                    <td className="text-center border">
                      <button
                        onClick={() => onRevertPressed(brand)}
                        className="hover:text-successColor transition-colors"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
