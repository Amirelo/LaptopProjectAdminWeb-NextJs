import { getAllBrands, getAllMemories, getAllOperSys, getAllProcessors, getAllScreens, getAllStorages, getProductImagesByProdID, insertProdct, updateProduct, uploadImageToImgur } from "@/services/AppService";
        ...
        isLoading == false ?
            <div className=" w-full h-full top-0 left-0 fixed">
                ...
                <div className="absolute w-1/2 left-1/3 top-8 h-fit rounded-2xl bg-inputBackgroundColor border border-inputBorderColor">
                    ...
                    <p className="ms-4 mt-2 font-bold">{item? "Current Product Image" : ""}</p>
                    <div className="flex flex-row w-full h-1/5 items-center p-6 flex-wrap justify-around">
                        
                        {listProductImages? listProductImages.map(prodImg => {
                            return (
                    ...
                    {listUploadImages.map((upFiles, key) => {
                            return (
                                <button key={key} className=" w-24 h-24 items-center justify-center flex bg-slate-200 rounded-xl transition-all">
                                    <Image width={100} height={100} src={URL.createObjectURL(upFiles)} alt={"Upload Product Image"} />
                                </button>
                            )
                        })}
                        </div>
                    ...
                    <div className="justify-center items-center flex flex-col">
                        <div className=" w-24 h-24 items-center justify-center flex bg-slate-200 rounded-xl">
                            <svg className="absolute w-10" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
                            </svg>
                            <input onChange={event =>uploadImageToClient(event)} type="file" className=" w-24 h-24 opacity-0 absolute"></input>
                        </div>
                        <p>Add Local Image</p>
                    </div>

                    <div className="flex-1 justify-center items-center flex flex-col">
                        <input className=" w-4/5 border border-black rounded-lg ps-2" placeholder="Add image link"></input>
                        <p>Add Image Link</p>
                    </div>
                    </div>
