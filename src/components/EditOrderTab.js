...
export default function EditOrderTab({
  onBackgroundPressed,
  onDeletePress,
  item,
}) {
  ...
  const onEditButtonPressed = async () => {
    const res = await updateUserOrderStatus(
      item.userOrderID,
      item.userID,
      itemStatus,
      statusArr[itemStatus].toUpperCase()
    );
    console.log(res);
    onDeletePress();
  };
  ...
  return (
    <div className=" w-full h-full top-0 left-0 absolute">
      ...
      <div className="absolute mt-20 w-1/2 left-1/3 top-8 h-fit rounded-2xl bg-secondaryColor border">
        ...
        <div className="flex flex-row items-center justify-center mx-4 mt-6">
          <p className="w-1/4 mr-2 text-white">Order status</p>
          <select
        ...
        <div className="flex flex-row items-center justify-around mt-6">
          ...
          <button
            disabled={item.status != itemStatus ? false : true}
            onClick={onEditButtonPressed}
            className={`${
              item.status != itemStatus
                ? "bg-successColor hover:font-bold"
                : "disabled:bg-successColor/50"
